...
ITEMS_PER_PAGE = 12
DIRECT_DOWNLOAD_PATTERN_IDS = {"LEOPARD001"}
...
def image_download_bytes(image: Image.Image, file_format: str) -> bytes:
    """Encode the exact unwatermarked tile for a customer download."""
    output = BytesIO()
...
def render_app() -> None:
    ...
    st.caption(
        f"Sharp original-tile preview · No watermark · {tile.width} × {tile.height} px"
    )
    ...
    if is_direct_download:
        ...
        st.success(
            "The preview and your downloaded JPEG and TIFF are unwatermarked."
        )
        st.caption(
