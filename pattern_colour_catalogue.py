...
def add_preview_watermark(canvas: Image.Image) -> Image.Image:
    """Add the protective preview watermark."""
    canvas_size = canvas.size
    overlay = Image.new("RGBA", canvas_size, (255, 255, 255, 0))
    draw = ImageDraw.Draw(overlay)
    font_size = max(20, min(canvas_size) // 38)
    try:
    ...
    x_step = text_width + max(90, canvas_size[0] // 12)
    y_step = max(font_size * 5, canvas_size[1] // 5)
    for row, y in enumerate(range(y_step // 2, canvas_size[1], y_step)):
        start_x = 24 if row % 2 == 0 else -(x_step // 2)
        for x in range(start_x, canvas_size[0], x_step):
            draw.text((x, y), WATERMARK_TEXT, fill=(45, 45, 45, 40), font=font)
...
def single_tile_preview(tile: Image.Image, canvas_size: tuple[int, int]) -> Image.Image:
    ...
    return add_preview_watermark(canvas)


def four_tile_preview(tile: Image.Image, canvas_size: tuple[int, int] = PREVIEW_SIZE) -> Image.Image:
    """Show one 2 × 2 repeat so the pattern remains large and sharp."""
    canvas = Image.new("RGB", canvas_size, tile.getpixel((0, 0)))
    tile_size = (canvas_size[0] // 2, canvas_size[1] // 2)
    resized = tile.resize(tile_size, Image.Resampling.LANCZOS)
...
def formspree_endpoint() -> str | None:
    try:
        value = st.secrets.get("FORMSPREE_ENDPOINT", "")
    except Exception:
        return None
    endpoint = str(value).strip()
    if not endpoint.startswith("https://formspree.io/f/"):
        return None
    return endpoint


def send_formspree_submission(fields: dict[str, str]) -> None:
    endpoint = formspree_endpoint()
    if not endpoint:
        raise RuntimeError("Form delivery is not activated.")
    ...
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "VanyaPatternColourStudio/1.0",
        },
        method="POST",
    )
    try:
        with urlopen(request, timeout=20) as response:
            if response.status not in {200, 201, 202}:
                raise RuntimeError(f"Formspree returned status {response.status}.")
    except HTTPError as exc:
        details = ""
        try:
            response_data = json.loads(exc.read().decode("utf-8"))
            details = response_data.get("error") or "; ".join(
                item.get("message", "") for item in response_data.get("errors", [])
            )
        except Exception:
            details = ""
        suffix = f" {details}" if details else ""
        raise RuntimeError(f"Formspree rejected the submission ({exc.code}).{suffix}") from exc
    except URLError as exc:
        raise RuntimeError("The form service could not be reached.") from exc
...
def render_app() -> None:
    ...
    st.write(
        ...
        "then send your chosen colour version."
    )
    ...
    preview = add_preview_watermark(tile)
    ...
    if selected_pattern.mode == "background":
        scope_text = "background colour changes only"
    else:
        scope_text = "colour changes only"
    st.info(
        "This complimentary service is only for selecting colours. "
        "Changes to motifs, layout, scale, spacing or other design elements "
        "are not included."
    )
    with st.form("send_selection_form"):
        customer_name = st.text_input("Your name", placeholder="Name or business name")
        customer_email = st.text_input("Your email", placeholder="you@example.com")
        ...
        confirmation_choice = st.radio(
            ...
            [
                "Yes, I confirm that I purchased this pattern individually "
                "from Vanya Illustrations and understand that this "
                f"complimentary service includes {scope_text}."
            ],
            index=None,
        )
        terms_confirmed = confirmation_choice is not None
        submitted = st.form_submit_button(
            "Send my colour selection",
            type="primary",
            use_container_width=True,
        )
    ...
    if submitted:
        ...
        if not clean_name:
            ...
            st.error("Please enter your Etsy order number.")
        elif not terms_confirmed:
            st.error("Please confirm the colour-service terms.")
        elif not formspree_endpoint():
            st.error("Form delivery is not activated yet. Please contact Vanya directly.")
        else:
            ...
            submission = {
                ...
                "palette": palette_name,
                "background": background_display,
                "line_or_motif": line_display,
                "etsy_order_number": order_number.strip(),
                "service_scope": scope_text,
                "eligibility_confirmed": "Yes",
            }

            try:
                with st.spinner("Sending your selection…"):
                    send_formspree_submission(submission)
            except Exception as exc:
                st.error(
                    f"Your selection could not be sent. {exc}"
                )
            else:
                st.success(
                    "Your colour selection was sent successfully. "
                    "Vanya can reply directly to your email."
                )
