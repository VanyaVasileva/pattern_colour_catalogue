...
def catalogue_thumbnail(pattern: Pattern) -> Image.Image:
    ...
    return four_tile_preview(tile, canvas_size=(480, 480))
...
def yahoo_app_password() -> str | None:
    try:
        value = st.secrets.get("YAHOO_APP_PASSWORD", "")
    except Exception:
        return None
...
def send_selection_email(
    ...
    if not password:
        raise RuntimeError("Email delivery is not activated.")
    ...
    with smtplib.SMTP_SSL("smtp.mail.yahoo.com", 465, context=context, timeout=20) as server:
...
def palette_controls(pattern: Pattern) -> tuple[str, str, str]:
    ...
    if "background_colour" not in st.session_state:
        st.session_state.background_colour = "#FCFBF8"
...
def render_app() -> None:
    ...
    preview = four_tile_preview(tile)
    ...
    if submitted:
        ...
        if not clean_name:
        ...
        elif not yahoo_app_password():
            st.error("Email delivery is not activated yet. Please contact Vanya directly.")
        else:
            ...
            try:
                ...
                st.error(
                    f"Yahoo rejected the sending-account login (error {exc.smtp_code}). "
                    "Please confirm that the app password belongs to "
                    f"{SENDER_EMAIL}."
                )
            except smtplib.SMTPSenderRefused as exc:
                st.error(
                    f"Yahoo rejected the sender address {SENDER_EMAIL} "
                    f"(error {exc.smtp_code}). Please check the exact spelling "
                    "of the Yahoo account."
                )
            except smtplib.SMTPRecipientsRefused:
                st.error(
                    f"Yahoo rejected the receiving address {CONTACT_EMAIL}. "
                    "Please check that this mailbox is active."
                )
            except smtplib.SMTPDataError as exc:
                st.error(
                    f"Yahoo accepted the login but rejected the email "
                    f"(error {exc.smtp_code}). Please wait a few minutes and try again."
                )
            except (TimeoutError, OSError):
                st.error(
                    "The email server could not be reached. "
                    "Please wait a moment and try again."
                )
            except Exception as exc:
                st.error(
                    "Your selection could not be sent. "
